 ...
 * text-only / single-thread case (see chunk-LPMZNXSF.js:3239 ff):
 *
 *   Dec 4, 2025:
 *   User (14:30): hello there
 *   Assistant (14:31): hi back
 *   Assistant: continuing (same time as above; time omitted on repeat)
 *   Dec 5, 2025:
 *   User (09:15): next day
 *
...
export function formatMessagesForObserver(messages: readonly ObservableMessage[]): string {
  ...
  for (const msg of messages) {
    ...
    if (!title) {
      out.push(timeLabel ? `${timeLabel}: ${body}` : body);
    } else {
      out.push(`${title}${timeLabel ? ` ${timeLabel}` : ''}: ${body}`);
    }
