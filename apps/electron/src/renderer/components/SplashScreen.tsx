...
export function SplashScreen({ isExiting, onExitComplete }: SplashScreenProps) {
  return (
    <motion.div
      className={`fixed inset-0 z-splash flex items-center justify-center bg-background ${isExiting ? 'pointer-events-none' : ''}`}
      initial={{ opacity: 1 }}
    ...
    >
      ...
      >
        <OrchaAgentsSymbol className="h-8 text-accent" />
      </motion.div>
    </motion.div>
  )
}
