...
- **`.isolated.ts`-Testdateien laufen NIE in `bun test`:** Die Naming-Konvention (repo-weit 5 Dateien, seit v0.5.0, vorbestehend upstream-artig) nimmt sie aus buns Test-Discovery — jedes „N pass"-Gate, das solche Dateien abdecken soll, hat sie nie ausgeführt (entdeckt beim bg-child-sessions-Hardening: die 79 Tests in `pre-tool-use-checks.isolated.ts` liefen nirgends). Workaround für neue Features: zusätzlich eine regulär entdeckte `*.test.ts` anlegen (so geschehen: `pre-tool-use-step0-routing.test.ts`). **Offen:** repo-weite Konvention prüfen/beheben — betrifft potenziell die Gate-Aussagekraft anderer Features.
